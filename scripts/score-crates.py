...
def get_crate_path(crate: str) -> Path:
    meta = json.loads(run(["cargo", "metadata", "--format-version", "1", "--no-deps"])[1])
    for pkg in meta.get("packages", []):
        if pkg["name"] == crate:
            return Path(pkg["manifest_path"]).parent
    return WORKSPACE_ROOT / "crates" / crate.replace("-", "_")
...
def score_stubs(crate: str) -> Tuple[int, str]:
    ...
    crate_path = get_crate_path(crate)
