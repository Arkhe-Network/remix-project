...
pub fn apply(state: State, event: &Event, hasher: &dyn Hasher) -> Result<State, TransitionError> {
    match event {
        ...
        Event::EvidenceAdded(id, art_id, c, sig, ts, ph) => apply_evidence(
            state,
            *id,
            *art_id,
            c.clone(),
...
fn apply_evidence(
    ...
    if state.evidences.contains_key(&id) {
        return Err(TransitionError::IdAlreadyExists);
    }
    if !state.artifacts.contains_key(&art_id) {
        return Err(TransitionError::ReferencedIdNotFound);
    }
    ...
    if !pre_hash_ok {
        return Err(TransitionError::InvalidParentHash);
    }
