...
impl QplAccelerator {
    ...
    pub fn execute_convolution(
        ...
        // No TLM single-threaded, a SRAM já está pronta. Evita deadlock.
        if !sram.is_ready(self.config.iterations as u64) {
        ...
        for (i, res) in results.iter_mut().enumerate().take(DOMAIN_NODES) {
            ...
            *res = QplResult { node: i, input, output };
        }
