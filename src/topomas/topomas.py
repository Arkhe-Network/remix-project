...
class MatminerEngine:
    ...
    def _get_featurizers(self):
        ...
        featurizers = [
            ...
            DensityFeatures(),
            RadialDistributionFunction(), # Gera muitas features
            StructuralHeterogeneity(),
            # AngularFourierSeries removed due to import error
        ]
    ...
    def featurize_many(self, structures: List[Structure]) -> np.ndarray:
        ...
        logger.info(f"Extraídas {feats_array.shape[1]} features reais.")
...
class HQCNNReal(nn.Module):
    ...
    def _one_hot(self, x, n_classes):
        # In SamplerQNN, interpret function maps integer bitstring to an output index.
        # It should return an integer, not a one-hot vector. SamplerQNN will accumulate probabilities internally.
...
class FeaturizerAgent(Agent):
    ...
    def __init__(self, name: str = "FeaturizerAgent", parquet_path: str = "features.parquet"):
        super().__init__(name)
        self.parquet_path = parquet_path

    def execute(self, state: Dict) -> Dict:
        if os.path.exists(self.parquet_path):
            logger.info(f"Carregando features do arquivo {self.parquet_path}...")
            df = pd.read_parquet(self.parquet_path)
            state['X_matminer'] = df.values.astype(np.float32)
            return state
        ...
        # Otimização agressiva de tipos de dados
        feats_array = feats_array.astype(np.float32)

        # Salvar em .parquet
        logger.info(f"Salvando features em {self.parquet_path}...")
        df = pd.DataFrame(feats_array)
...
if __name__ == "__main__":
    if not HAS_PMATGEN:
        logger.error("Execute: pip install pymatgen matminer networkx qiskit-machine-learning transformers pyarrow")
        exit(1)
    ...
    demo_structs = [
        ...
        Structure(Lattice.cubic(5.4), ["Si", "Si"], [[0,0,0], [0.25,0.25,0.25]]),
