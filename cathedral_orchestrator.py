...
class CathedralHandler(http.server.SimpleHTTPRequestHandler):
    ...
    MAX_BODY_SIZE = 10 * 1024 * 1024  # 10 MB limit for POST body
    ...
    def do_GET(self):
        if self.path == '/api/metrics':
            ...
            # Disable arbitrary file reading except for allowed static files if necessary,
            # or simply return 404 for any unhandled paths to prevent information disclosure.
            self._json_response({"error": "Not found"}, 404)

    def do_POST(self):
        length_header = self.headers.get('Content-Length')
        if not length_header:
            self._json_response({"error": "Length Required"}, 411)
            return
